<script>
  ...
  /**
   * articles: Array of article objects with title, slug, image, publishedAt, etc.
   */
  let { articles = [] } = $props();
      ...
      <div class="flex gap-4 min-w-max">
        {#each articles as article}
          <a
            ...
            class="w-80 flex-shrink-0 bg-white dark:bg-gray-900 rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 group"
          >
            <!-- Image Section -->
            <div class="relative aspect-square">
              {#if article.image}
                {#if isSanityImage(article.image)}
                  <Image
                    url={article.image}
